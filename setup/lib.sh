...
function wait_for_elasticsearch {
	...
	for _ in $(seq 1 60); do
		...
		# Accept 200 (authenticated) or 401 (Elasticsearch is running but auth required)
		# Both mean Elasticsearch is healthy and responding
		if [[ "${output: -3}" -eq 200 ]] || [[ "${output: -3}" -eq 401 ]]; then
			result=0
			break
		fi
